    ...
    <DashboardLayout title={t.equipment.title} requiredRole="super_admin">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search equipment..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border text-sm input-accent" style={{ ...inputStyle }} />
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20" style={{ color: "#8b93a1" }}><Package className="w-12 h-12 mx-auto mb-4 opacity-30" /><p>{t.common.noData}</p></div>
        ) : (
            ...
            {items.map((item, i) => (
              <div key={item.id} className="flex items-center gap-4 px-5 py-4 hover:bg-[#23272e] transition-colors" style={{ borderBottom: i < items.length - 1 ? "1px solid #23272e" : "none" }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: "#8b93a1" }}>{item.nameAr}</p>
                </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editTarget ? t.equipment.editEquipment : t.equipment.createEquipment}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => setField("isActive", !form.isActive)} className="w-10 h-6 rounded-full transition-all" style={{ background: form.isActive ? "#c8f323" : "#2d323a" }}>
